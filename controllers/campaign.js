  ...
  createCampaign: async (req, res) => {
    ...
    */
    try {
      ...
      const stageCreate = await StageController.createStageByPasses(
        campaign.id,
        stageNum
      );
    ...
    */
    try {
      ...
      if (!email || !usertype) {
        const resp = responseWrapper(
          null,
          "email and usertype is required",
          400
        );
        return res.status(400).json(resp);
      }
  ...
  deleteCampaignById: async (req, res) => {
    ...
    */
    try {
      ...
      await awsS3.deleteObjectsFromS3ForCampAndOrg(campaignData.id.toString());
    ...
    */
    try {
      ...
      if (!id || !status) {
        const resp = responseWrapper(
          null,
          "campaign id and status are required",
          400
        );
        return res.status(400).json(resp);
      }
